    ...
    <div dir="rtl" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, padding: '0 18px 10px', position: 'relative', zIndex: 10, maxWidth: '580px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px', marginTop: '8px' }}>
          <button onClick={onBack} style={{ padding: '12px 18px', borderRadius: '12px', background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.6)', fontFamily: FONT, fontWeight: 700, fontSize: '15px', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', transition: 'all 0.2s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.12)'; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.07)'; }}>
            ← חזרה
          </button>
          ...
          {level.exercises.map((_, idx) => {
            ...
            return (
              <button
                ...
                style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderRadius: '15px', background: isCurrent ? 'rgba(251,146,60,0.14)' : exProg?.completed ? 'rgba(52,211,153,0.07)' : 'rgba(255,255,255,0.06)', border: `1px solid ${isCurrent ? 'rgba(251,146,60,0.48)' : exProg?.completed ? 'rgba(52,211,153,0.25)' : 'rgba(255,255,255,0.09)'}`, backdropFilter: 'blur(12px)', boxShadow: isCurrent ? '0 0 22px rgba(251,146,60,0.13)' : 'none', cursor: 'pointer', transition: 'all 0.15s', textAlign: 'right' }}
                onMouseEnter={e => { if (!isCurrent) { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.1)'; (e.currentTarget as HTMLElement).style.border = '1px solid rgba(255,255,255,0.16)'; } }}
                onMouseLeave={e => { if (!isCurrent) { (e.currentTarget as HTMLElement).style.background = exProg?.completed ? 'rgba(52,211,153,0.07)' : 'rgba(255,255,255,0.06)'; (e.currentTarget as HTMLElement).style.border = `1px solid ${exProg?.completed ? 'rgba(52,211,153,0.25)' : 'rgba(255,255,255,0.09)'}`; } }}
              >
                <div style={{ width: '38px', height: '38px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontWeight: 900, fontSize: '15px', background: isCurrent ? 'linear-gradient(135deg, #fb923c, #f59e0b)' : exProg?.completed ? 'rgba(52,211,153,0.22)' : 'rgba(255,255,255,0.09)', color: isCurrent ? '#1a0800' : exProg?.completed ? '#34d399' : 'rgba(255,255,255,0.5)', boxShadow: isCurrent ? '0 0 16px rgba(251,146,60,0.45)' : 'none' }}>
                  {exProg?.completed ? '✓' : idx + 1}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, color: '#fff', fontSize: '16px', fontFamily: FONT }}>תרגיל {idx + 1}</div>
                  <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', fontFamily: FONT, marginTop: '2px' }}>{exProg?.completed ? 'הושלם' : 'לחץ לנגן'}</div>
                </div>
                {exProg?.stars !== undefined && exProg.stars > 0 && <StarRating stars={exProg.stars} size="sm" />}
              </button>
            );
          })}
        </div>
